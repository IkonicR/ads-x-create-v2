...
import { Send, Bot, User, Sparkles, Paperclip, Download } from 'lucide-react';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ business }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiResponse = await sendChatMessage(business, historyForApi, userMsg.text);
      ...
      >
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              ...
              <div className={`max-w-[80%] flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`p-4 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
                  msg.role === 'ai'
                    ...
                    : `${styles.bg} ${styles.shadowIn} ${styles.textMain} border-l-4 border-[#6D5DFC]`
                }`}>
                  {msg.text}
                </div>
                ...
                {msg.attachment && msg.attachment.type === 'image' && (
                  <div className={`mt-4 rounded-2xl overflow-hidden p-2 ${styles.bg} ${styles.shadowOut} animate-fade-in max-w-sm`}>
                     ...
                     <div className={`mt-2 px-2 flex justify-between items-center ${styles.textSub} text-xs`}>
                       ...
                       <Sparkles size={12} className="text-[#6D5DFC]" />
                     </div>
                  </div>
                )}
              </div>
          ...
          {loading && (
             <div className="flex gap-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${styles.bg} ${styles.shadowOut} text-[#6D5DFC]`}>
                   <Bot size={20} />
                </div>
                <div className={`p-4 rounded-2xl ${styles.bg} ${styles.shadowOut} text-sm`}>
                   <div className="flex gap-1">
                     <div className="w-2 h-2 bg-[#6D5DFC] rounded-full animate-bounce"></div>
                     <div className="w-2 h-2 bg-[#6D5DFC] rounded-full animate-bounce delay-100"></div>
                     <div className="w-2 h-2 bg-[#6D5DFC] rounded-full animate-bounce delay-200"></div>
                   </div>
                </div>
             </div>
          )}
        ...
        <div className={`p-4 ${styles.bgAccent} border-t ${styles.border}`}>
          <div className="flex gap-4 items-end">
            <button className={`p-3 rounded-xl ${styles.bg} ${styles.shadowOut} ${styles.textSub} hover:text-[#6D5DFC] transition-colors mb-1`}>
               <Paperclip size={20} />
            </button>
            <div className="flex-1 relative">
                <textarea 
                  className={`w-full rounded-xl px-4 py-3 pr-12 outline-none transition-all resize-none min-h-[50px] max-h-[120px] ${styles.bg} ${styles.shadowIn} ${styles.textMain} ${styles.inputPlaceholder} focus:ring-2 focus:ring-[#6D5DFC]/20`}
                ...
                <div className="absolute right-3 bottom-3">
                   <Sparkles size={16} className="text-[#6D5DFC] opacity-50" />
                </div>
            </div>
            <NeuButton 
              variant="primary" 
