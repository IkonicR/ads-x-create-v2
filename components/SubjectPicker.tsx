...
export const SubjectPicker: React.FC<SubjectPickerProps> = ({ business, onSelect, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
            <NeuCard className="w-full max-w-lg max-h-[80vh] flex flex-col p-0 overflow-hidden shadow-2xl">

                {/* Header */}
                <div className={`p-4 border-b ${styles.border} flex justify-between items-center bg-opacity-50`}>
                    <h3 className={`text-lg font-bold ${styles.textMain}`}>Select Subject</h3>
                    <button onClick={onClose} className={`p-2 rounded-full hover:bg-black/5 ${styles.textSub}`}>
                        <X size={20} />
                    </button>
                </div>
